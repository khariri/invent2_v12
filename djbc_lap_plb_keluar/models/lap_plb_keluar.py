...
class DJBCPLBKeluar(models.Model):
    _name = 'djbc.lap_plb_keluar'
    ...
    def init(self):
        self.env.cr.execute("""
        DROP FUNCTION IF EXISTS djbc_lap_plb_keluar(DATE, DATE);
        CREATE OR REPLACE FUNCTION djbc_lap_plb_keluar(date_start DATE, date_end DATE)
...
BEGIN
	delete from djbc_lap_plb_keluar;
	-- v_wh='WH/Stock';
	
	for rec in csr loop
		insert into djbc_lap_plb_keluar (no_dok, tgl_dok,jenis_dok,no_pengeluaran, tgl_pengeluaran, penerima, kode_barang,
