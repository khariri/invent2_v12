...
class DJBCPLBMasuk(models.Model):
    _name = 'djbc.lap_plb_masuk'
    _description = 'DJBC Laporan Pemasukan PLB'
    _rec_name = 'no_dok'
    ...
    def init(self):
        self.env.cr.execute("""
        DROP FUNCTION IF EXISTS djbc_lap_plb_masuk(DATE, DATE);
        CREATE OR REPLACE FUNCTION djbc_lap_plb_masuk(date_start DATE, date_end DATE)
...
BEGIN
	delete from djbc_lap_plb_masuk;
	-- v_wh='WH/Stock';
	
	for rec in csr loop
		insert into djbc_lap_plb_masuk (no_dok, tgl_dok,jenis_dok,no_penerimaan, tgl_penerimaan, pengirim, kode_barang,
