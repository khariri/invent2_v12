...
class DJBCNofasKeluarV3(models.Model):
    _name = 'djbc.nofas_keluar_v3'
    _description = 'DJBC Laporan Pengeluaran'
    _rec_name = 'no_dok'
    ...
    no_pengeluaran = fields.Many2one(comodel_name="stock.picking", string="Nomor Pengeluaran", required=False, )
    ...
    def init(self):
        self.env.cr.execute("""
        DROP FUNCTION IF EXISTS djbc_nofas_keluar_v3(DATE, DATE);
        CREATE OR REPLACE FUNCTION djbc_nofas_keluar_v3(date_start DATE, date_end DATE)
...
DECLARE
	...
	csr cursor for
        ...
        join (select z1.id as id, 
            ...
            from stock_move z1 
            join stock_picking w1 on z1.picking_id=w1.id) yy 
        on yy.id =xx.id
		where xx.state='done' 
...
BEGIN
	delete from djbc_nofas_keluar_v3;
	-- v_wh='WH/Stock';
	
	for rec in csr loop
		insert into djbc_nofas_keluar_v3 (no_dok, tgl_dok,jenis_dok,no_pengeluaran, tgl_pengeluaran, penerima, kode_barang,
