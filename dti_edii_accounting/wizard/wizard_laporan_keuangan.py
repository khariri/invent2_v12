...
class DtWizardLaporanKeuangan(models.TransientModel):
    _name               = "dt.wizard.laporan.keuangan"
    _description        = "Laporan Keuangan"
    ...
    def button_download_excel(self):
        ...
        if self.account_report_id.name == 'BALANCE SHEET':
            ...
            worksheet1.merge_range('B1:G1', 'Printed by ' +  datetime.strptime(today, "%Y-%m-%d %H:%M:%S").\
                                                 strftime("%Y-%m-%d %H:%M:%S") + ' by ' + self.env.user.name, left_title)
            worksheet1.merge_range('B2:G2', self.env.user.company_id.name, h1_center_title)
            worksheet1.merge_range('B3:G3', 'Balance Sheet Report', h1_center_title)
            worksheet1.merge_range('B4:G4', 'Period : ' + self.period_id.name, center_title)
            
            row = 5
            worksheet1.write(row, 1, '', h1_content_format_center)
            worksheet1.write(row, 2, '', h1_content_format_center)
            worksheet1.write(row, 3, 'Beginning Year', h1_content_format_right)
            worksheet1.write(row, 4, 'Until Prev. Period', h1_content_format_right)
            worksheet1.write(row, 5, 'This Period', h1_content_format_right)
            worksheet1.write(row, 6, 'Year to Date (YTD)', h1_content_format_right)

            row += 1

            data_report = self.env['dt.acc.global.function'].get_balance_sheet_report_data(self.company_id.id, self.fiscalyear_id.id, self.period_id.id, self.is_detail, self.with_movement)

            for dt in data_report[0]:
                if dt['name'] != 'BREAK':

                    if dt['style'] == 1:
                        worksheet1.write(row, 1, dt['code'] or '', content_format_style1)
                        worksheet1.write(row, 2, dt['name'], content_format_style1)
                        worksheet1.write(row, 3, dt['saldo_awal_tahun'] if dt['type'] != 'view' else '', numb_format_style1)
                        worksheet1.write(row, 4, dt['saldo_sd_bulan_lalu'] if dt['type'] != 'view' else '', numb_format_style1)
                        worksheet1.write(row, 5, dt['mutasi_bulan_ini'] if dt['type'] != 'view' else '', numb_format_style1)
                        worksheet1.write(row, 6, dt['saldo_sd_bulan_ini'] if dt['type'] != 'view' else '', numb_format_style1)
                    elif dt['style'] == 2:
                        worksheet1.write(row, 1, dt['code'] or '', content_format_style2)
                        worksheet1.write(row, 2, dt['name'], content_format_style2)
                        worksheet1.write(row, 3, dt['saldo_awal_tahun'] if dt['type'] != 'view' else '', numb_format_style2)
                        worksheet1.write(row, 4, dt['saldo_sd_bulan_lalu'] if dt['type'] != 'view' else '', numb_format_style2)
                        worksheet1.write(row, 5, dt['mutasi_bulan_ini'] if dt['type'] != 'view' else '', numb_format_style2)
                        worksheet1.write(row, 6, dt['saldo_sd_bulan_ini'] if dt['type'] != 'view' else '', numb_format_style2)
                    elif dt['style'] == 3:
                        worksheet1.write(row, 1, dt['code'] or '', content_format_style3)
                        worksheet1.write(row, 2, dt['name'], content_format_style3)
                        worksheet1.write(row, 3, dt['saldo_awal_tahun'] if dt['type'] != 'view' else '', numb_format_style3)
                        worksheet1.write(row, 4, dt['saldo_sd_bulan_lalu'] if dt['type'] != 'view' else '', numb_format_style3)
                        worksheet1.write(row, 5, dt['mutasi_bulan_ini'] if dt['type'] != 'view' else '', numb_format_style3)
                        worksheet1.write(row, 6, dt['saldo_sd_bulan_ini'] if dt['type'] != 'view' else '', numb_format_style3)
                    else:
                        worksheet1.write(row, 1, dt['code'] or '', content_format_style4)
                        worksheet1.write(row, 2, dt['name'], content_format_style4)
                        worksheet1.write(row, 3, dt['saldo_awal_tahun'] if dt['type'] != 'view' else '', numb_format_style4)
                        worksheet1.write(row, 4, dt['saldo_sd_bulan_lalu'] if dt['type'] != 'view' else '', numb_format_style4)
                        worksheet1.write(row, 5, dt['mutasi_bulan_ini'] if dt['type'] != 'view' else '', numb_format_style4)
                        worksheet1.write(row, 6, dt['saldo_sd_bulan_ini'] if dt['type'] != 'view' else '', numb_format_style4)

                row += 1
            row += 2

            t_passiva_saldo_awal_tahun = 0
            t_passiva_saldo_sd_bulan_lalu = 0
            t_passiva_mutasi_bulan_ini = 0
            t_passiva_saldo_sd_bulan_ini = 0

            for dt in data_report[1]:
                if dt['name'] != 'BREAK':
            ...
            return{
                ...
                'url': 'web/content/?model=dt.wizard.laporan.keuangan&field=file&download=true&id=%s&filename=BalanceSheetReport.xlsx'%(self.id),
                'target': 'new',
            }
        ...
        # PROFIT AND LOSS REPORT
            ...
            worksheet1.merge_range('B3:F3', 'Profit & Loss Report', h1_center_title)
            ...
            data_report = self.env['dt.acc.global.function'].get_profit_loss_report_data(self.company_id.id, self.fiscalyear_id.id, self.period_id.id, self.is_detail, self.with_movement)
            ...
            return{
                'type' : 'ir.actions.act_url',
                'url': 'web/content/?model=dt.wizard.laporan.keuangan&field=file&download=true&id=%s&filename=ProfitLossReport.xlsx'%(self.id),
                'target': 'new',
            }